...
import {subCategories} from '../../redux/slices/subCategory';
...
const Playlist = React.memo(
  ...
  any) => {
    ...
    return (
      <TouchableOpacity
        onPress={() => {
          //   if (subStatus == 'free' && isPaid == 'paid') {
          //     setShowPopUp(true);
          //   } else {
          navigation.navigate(SCREENS.PLAYLISTS, {
            itemId: id,
            name: title,
            url: url,
          });
...
const SubCategory = ({route, navigation}: any) => {
  ...
  const playlistsData = useAppSelector((state: any) => state.subCats.data);
  const isLoading = useAppSelector((state: any) => state.subCats.loading);
  const subCatPlaylistParams = `?parent=${itemId}`;
  // const [showPopUp, setShowPopUp] = useState(false);
  // const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    dispatch(subCategories(subCatPlaylistParams));
  }, []);
